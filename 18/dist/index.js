...
const initGrid = function (instructions) {
    let maxLeft = 0;
    let maxRight = 0;
    let currentX = 0;
    let maxUp = 0;
    ...
    const startPos = { x: -maxLeft, y: -maxUp };
    let grid = [];
...
const followInstructions = function () {
    ...
    instructions.forEach((instruction) => {
        if (instruction.direction === "U") {
            for (let y = currentPosition.y - 1; y >= currentPosition.y - instruction.steps; y--) {
        ...
        if (instruction.direction === "L") {
            for (let x = currentPosition.x - 1; x >= currentPosition.x - instruction.steps; x--) {
...
const getAnswer = function (grid) {
    return grid.flat().filter(isNotBlank).length;
};
function isNotBlank(cell) {
    return cell !== ".";
}
