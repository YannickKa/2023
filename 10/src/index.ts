...
class Graph {
  ...
  addEdge(srcNode: GraphNode, dstNode: GraphNode): void {
    this.nodeMap.get(srcNode.id)?.push(dstNode);
  }
  ...
  countLoop(start: string): [number, string[]] {
    ...
    while (!visited.includes(currentNode)) {
      ...
      const touchingNodes = this.nodeMap.get(currentNode);
      if (touchingNodes && !visited.includes(touchingNodes[0].id)) {
        currentNode = touchingNodes[0].id;
        count++;
      } else if (touchingNodes && !visited.includes(touchingNodes[1].id)) {
        currentNode = touchingNodes[1].id;
        count++;
      }
    }
...
const fillNodes = (graph: Graph): Graph => {
  for (let i = 0; i < grid.length; i++) {
    for (let j = 0; j < grid[0].length; j++) {
      const point: Point = { x: i, y: j };
      const id: string = `${i}|${j}`;
      const symbol: string = grid[i][j];
      const node: GraphNode = { id: id, point: point, symbol: symbol };
      graph.addNode(id);
      fillEdges(graph, node);
    }
  }
...
const fillEdges = (graph: Graph, node: GraphNode): Graph => {
  const x: number = node.point.x;
  const y: number = node.point.y;

  if (node.symbol == "F") {
    const node1id: string = `${x + 1}|${y}`;
    const node1point: Point = { x: x + 1, y: y };
    const node1symbol: string = grid[x + 1]?.[y];
    ...
    const node2id: string = `${node.point.x}|${node.point.y + 1}`;
    const node2point: Point = { x: x, y: y + 1 };
    const node2symbol: string = grid[x]?.[y + 1];
    ...
    const node1id: string = `${node.point.x}|${node.point.y - 1}`;
    const node1point: Point = { x: x, y: y - 1 };
    const node1symbol: string = grid[x]?.[y - 1];
    const node1: GraphNode = {
      id: node1id,
      point: node1point,
      symbol: node1symbol,
    ...
    const node2id: string = `${node.point.x + 1}|${node.point.y}`;
    const node2point: Point = { x: x + 1, y: y };
    const node2symbol: string = grid[x + 1]?.[y];
    const node2: GraphNode = {
      id: node2id,
      point: node2point,
      symbol: node2symbol,
    ...
    const node1id: string = `${node.point.x - 1}|${node.point.y}`;
    const node1point: Point = { x: x - 1, y: y };
    const node1symbol: string = grid[x - 1]?.[y];
    const node1: GraphNode = {
      id: node1id,
