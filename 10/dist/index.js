...
const fillNodes = (graph) => {
    for (let i = 0; i < grid.length; i++) {
        for (let j = 0; j < grid[0].length; j++) {
            const point = { x: i, y: j };
            const id = `${i}|${j}`;
            const symbol = grid[i][j];
            const node = { id: id, point: point, symbol: symbol };
            graph.addNode(id);
...
const fillEdges = (graph, node) => {
    const x = node.point.x;
    const y = node.point.y;
    if (node.symbol == "F") {
        const node1id = `${x + 1}|${y}`;
        const node1point = { x: x + 1, y: y };
        const node1symbol = grid[x + 1]?.[y];
        const node1 = {
            id: node1id,
            point: node1point,
            symbol: node1symbol,
        };
        const node2id = `${node.point.x}|${node.point.y + 1}`;
        const node2point = { x: x, y: y + 1 };
        const node2symbol = grid[x]?.[y + 1];
        const node2 = {
            id: node2id,
            point: node2point,
            symbol: node2symbol,
        ...
        const node1id = `${node.point.x}|${node.point.y - 1}`;
        const node1point = { x: x, y: y - 1 };
        const node1symbol = grid[x]?.[y - 1];
        const node1 = {
            id: node1id,
            point: node1point,
            symbol: node1symbol,
        ...
        const node2id = `${node.point.x + 1}|${node.point.y}`;
        const node2point = { x: x + 1, y: y };
        const node2symbol = grid[x + 1]?.[y];
        const node2 = {
            id: node2id,
            point: node2point,
            symbol: node2symbol,
        ...
        const node1id = `${node.point.x - 1}|${node.point.y}`;
        const node1point = { x: x - 1, y: y };
        const node1symbol = grid[x - 1]?.[y];
        const node1 = {
            id: node1id,
